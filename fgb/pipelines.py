...
class FfgbPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('fgb.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `fgb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM fgb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO fgb (date, title, link, content)"
